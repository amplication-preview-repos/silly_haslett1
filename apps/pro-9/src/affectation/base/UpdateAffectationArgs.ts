/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { AffectationWhereUniqueInput } from "./AffectationWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { AffectationUpdateInput } from "./AffectationUpdateInput";

@ArgsType()
class UpdateAffectationArgs {
  @ApiProperty({
    required: true,
    type: () => AffectationWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => AffectationWhereUniqueInput)
  @Field(() => AffectationWhereUniqueInput, { nullable: false })
  where!: AffectationWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => AffectationUpdateInput,
  })
  @ValidateNested()
  @Type(() => AffectationUpdateInput)
  @Field(() => AffectationUpdateInput, { nullable: false })
  data!: AffectationUpdateInput;
}

export { UpdateAffectationArgs as UpdateAffectationArgs };
